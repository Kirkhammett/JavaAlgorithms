...
class Graph<E> {
	...
	private float[] dijkstra(int from, int path[]) {
				...
				if (!finalno[pom.node.getIndex()]) {
					if (distance[pom.node.getIndex()] <= 0) {
						distance[pom.node.getIndex()] = distance[from]
								+ pom.weight;
						path[pom.node.getIndex()] = from;
					} else {
						if (distance[from] + pom.weight < distance[pom.node
								.getIndex()]) {
							distance[pom.node.getIndex()] = distance[from]
									+ pom.weight;
							path[pom.node.getIndex()] = from;
			...
			for (int j = 0; j < this.num_nodes; j++) {
				if (!finalno[j] && distance[j] != -1) {
					if (!minit) {
						...
						if (minc > distance[j] && distance[j] > 0) {
							minc = distance[k = j];
						}
					}
				}
	...
	float min_path(int from, int to) {
		...
		int path[] = new int[this.num_nodes];
		ArrayStack<String> s = new ArrayStack<String>(100);
		float[] f = this.dijkstra(from, path);
		int index = to;
		while (index >= 0) {
			s.push(this.adjList[index].getInfo().toString());
			index = path[index];
		}
		String str = "";
		while (!s.isEmpty()) {
