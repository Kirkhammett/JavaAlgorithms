...
public class ConsecutiveNumbers {
	...
	public static boolean ConsecutiveNumbers(BTree<Integer> tree) {
		...
		BNode<Integer> now = node;
		BNode<Integer> next = tree.predecessorInorder(node);
		while(next.info != null) {
			int raz = now.info - next.info;
			if(raz != 1) {
				return false;
			}
			now = next;
			next = tree.predecessorInorder(now);
		}
		now = node;
		next = tree.successorInorder(node);
		while(next.info != null) {
			int raz = next.info - now.info;
			if(raz != 1) {
				return false;
			}
			now = next;
			next = tree.successorInorder(now);
		}
		return true;
	}
    ...
    public static void main(String[] args) throws Exception {
        ...
        // vasiot kod ovde
        System.out.println(ConsecutiveNumbers(tree));
