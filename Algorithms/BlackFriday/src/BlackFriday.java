...
public class BlackFriday {
	...
	public static void main(String[] args) throws Exception {
		...
		for (i = 0; i < N; i++) {
			...
			visits[i] = new Visit(st.nextToken(), Integer.parseInt(st
					.nextToken()));
		}

		// vasiot kod stoi ovde

		Heap<Visit> heap = new Heap<Visit>(N);
		for (i = 0; i < N; i++) {
			heap.insert(visits[i]);
		}
		heap.heapSort();
		int max = 0;
		for (i = N - 1; i >= 0; i--) {
			int count = 1;
			for (j = i - 1; j >= 0; j--) {
				int prv = heap.getAt(i).minutesEnd;
				int vtor = heap.getAt(j).minutesStart;
				if(prv >= vtor) {
					count++;
				}
			}
