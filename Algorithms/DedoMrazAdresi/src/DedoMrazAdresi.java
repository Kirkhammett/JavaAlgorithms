...
public class DedoMrazAdresi {
	public static void main (String[] args) throws IOException {
		...
		for (int i = 0; i < N; i++) {
			...
			tabela.insert(imeDobriDeca[i], adresaDobriDeca[i]);

		}

		int M = Integer.parseInt(br.readLine());
		CBHT<String, String> tabela1 = new CBHT<String, String>(M+1);// Vie ja odreduvate goleminata na tabelata
		...
		for (int i = 0; i < M; i++) {
			...
			staroIme[i] = del[0];
			novoIme[i] = del[1];
			tabela1.insert(staroIme[i], novoIme[i]);
		}

		//tuka se zapishuva imeto na deteto shto treba da se proveri
		String deteZaProverka = br.readLine();
		if(tabela.search(deteZaProverka)!=null)
		{
			SLLNode<MapEntry<String, String>> a= tabela.search(deteZaProverka);
			String adresa = a.element.value;
			String[] splitnata=adresa.split(" ");
			//System.out.println(splitnata);
			String staraAdresa=splitnata[0];
			if(tabela1.search(staraAdresa)!=null)
			{
				SLLNode<MapEntry<String, String>> b= tabela1.search(staraAdresa);
				String nova = b.element.value;
				nova +=" ";
				for(int i = 1; i <splitnata.length; i++)
				{
					nova += splitnata[i];
					nova += " ";
				}
